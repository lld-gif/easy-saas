...
function renderMarkdown(md: string): string {
  return md
    ...
    .replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" class="text-orange-400 underline" target="_blank" rel="noopener">$1</a>')
...
export function PackageSection({ ideaId }: PackageSectionProps) {
    ...
    return (
      <div className="mt-8">
        ...
        <div className="text-center mt-4">
          <p className="text-sm text-zinc-400 mb-3">Sign in to unlock the full package</p>
  ...
  if (subscriptionStatus !== "pro") {
    return (
      <div className="mt-8">
        ...
        <BlurredPreview />
        <div className="text-center mt-4">
          <p className="text-sm text-zinc-400 mb-3">Upgrade to Pro to unlock Tech Spec, Brand Kit, and Launch Checklist</p>
          <UpgradeButton />
        </div>
      </div>
    )
      ...
      <div className="mt-8">
        ...
        <div className="space-y-6">
          {[
            ...
            <details key={section.title} className="border border-zinc-800 rounded-xl overflow-hidden" open>
              <summary className="px-6 py-4 bg-zinc-800/50 cursor-pointer font-semibold text-zinc-100 flex items-center gap-2 hover:bg-zinc-800">
                <span>{section.icon}</span> {section.title}
              </summary>
              <div className="px-6 py-4 text-sm text-zinc-300 leading-relaxed"
  ...
  return (
    <div className="mt-8">
      <h2 className="text-xl font-bold mb-4">Quick Start Package</h2>
      <div className="border border-dashed border-zinc-700 rounded-xl p-8 text-center bg-zinc-900">
        <p className="text-zinc-300 mb-1">Get a personalized Tech Spec, Brand Kit, and Launch Checklist</p>
        <p className="text-xs text-zinc-500 mb-4">Generated by AI in ~10 seconds. Cached forever.</p>
