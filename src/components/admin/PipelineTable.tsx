          ...
          <tbody className="divide-y divide-gray-100">
            {runs.map((run) => (
              <tr key={run.id} className="hover:bg-gray-50">
                ...
                <td className="py-2 text-xs text-gray-600">{formatTime(run.finished_at)}</td>
                <td className="py-2">
                  {run.status === "success" ? (
                ...
                <td className="py-2 text-right text-gray-500">{run.ideas_duplicate}</td>
                <td className="py-2 text-right text-red-500">{run.ideas_error > 0 ? run.ideas_error : "—"}</td>
                <td className="py-2 text-right text-xs text-gray-500">{formatDuration(run.duration_ms)}</td>
              </tr>
            ))}
          </tbody>
        </table>
